##
# @file comRefbox.py
# @brief Modul komunikasi TCP dengan Refbox (Referee Box) untuk simulasi robot sepak bola.
#
# Modul ini bertanggung jawab untuk membuat koneksi ke Refbox menggunakan socket TCP,
# menerima dan memproses perintah pertandingan baik dalam bentuk string biasa maupun
# format JSON. Hasil komunikasi digunakan untuk mengatur status pertandingan di `varGlobals`.
#
# Modul ini juga mengatur identifikasi tim, start dan stop pertandingan, serta event-event lain
# seperti kickoff, goal, penalty, dan sebagainya.
#
...
#############################################################################################
##
# @brief Membuat koneksi TCP ke Refbox dan mengatur thread penerima pesan.
#
# Fungsi ini mencoba terhubung ke Refbox melalui IP dan PORT dari `varGlobals`.
# Jika berhasil menerima pesan "WELCOME", maka thread penerima (string/JSON) akan dimulai.
#
def connect_refbox():
    #MEMBUAT SOCKET
...
#############################################################################################
##
# @brief Thread penerima data string dari Refbox selama pertandingan.
#
# Fungsi ini berjalan terus selama `varGlobals.ref` aktif, menerima pesan dan mengatur
# nilai `MESSAGE_REFBOX` berdasarkan kode karakter Refbox (misal 'S' untuk STOP).
#
def recRef():
    data=bytearray(2)
...
#############################################################################################
##
# @brief Thread penerima data JSON dari Refbox selama pertandingan.
#
# Fungsi ini menerima perintah dalam format JSON dan mengatur status pertandingan
# berdasarkan isi dari field "command" dan "targetTeam".
#
def recRefJSON():
    #LOOPING
    while varGlobals.ref:
